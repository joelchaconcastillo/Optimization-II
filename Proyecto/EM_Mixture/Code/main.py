...
n = 100
d = 1
mycov = [[1, 0.0], [0.0, 1]] ##Covariance matrix...
mycov2 = [[d, 0.0], [0.0, d]] ##Covariance matrix...
mu1 = 16.66#[50, 50]
mu2 = 83#[10, 10]
#np.random.seed(20)
##Generating the data...
...
x1 = np.random.normal( mu1, 1, n)
x2 = np.random.normal( mu2, 1, n)
...
minX, maxX, minY, maxY = -10., 110., -10., 110.
m = 10#np.shape(x)[0]# 20 # the number  of densities too consider
x = np.linspace(minX, maxX, m)#, (maxX-minX).+1)
y = np.linspace(minY, maxY, np.sqrt(m))#, (maxY-minY)/200.+1)
print x
L = np.zeros((N, m))
sqrtm = m#nt(np.sqrt(m))
for i in range (0, N):
  for j in range(0,sqrtm):
   #for k in range(0,sqrtm):
    #L[i, j*sqrtm + k] = multivariate_normal.pdf(data[i,:], mean= [x[j], y[k]] , cov=mycov2);
    L[i, j] = 10*norm.pdf(data[i]-x[j]);
#    L[i, j] = norm.pdf(data[i]-x[j]);
