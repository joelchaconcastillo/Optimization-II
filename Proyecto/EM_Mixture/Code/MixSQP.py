...
#    gt = -(1.0/n)*( np.matmul(L.T,dt)) + np.ones(m)  ##compute approximate gradient
    ...
    if Method == 'PI':
      y = X + Pt
      y = PredictorCorrectorQPSolver(csc_matrix(Ht),csc_matrix(np.diag(np.ones(m))),np.zeros(m),at,y, 20, Epsilon, Epsilon, 0.3)[0] #... #solve quadratic subproblem... either interior point method or active set method
    else:
