...
def processImage(D, inImage):
   ...
   for i in range(0,Hi-Lw):
     ...
     #    outImage[i:i+Lw,j:j+Lw] = np.reshape(flatwindow,(Lw, Lw))
       if j > 0:
        outImage[i:i+Lw,j:j+Lw-1] = (flatwindow[0:Lw,0:Lw-2] + outImage[i:i+Lw,j-1:j+Lw-2]) /2.0
       #  outImage[i:i+Lw,j:j+Lw] = (np.reshape(flatwindow,(Lw, Lw)) + outImage[i+Lw,j:j+Lw])
...
inImagef = inImagef[100:130,100:130]
Nw = 50 #Dictionary size..
Lw = 6 #width size of each window
