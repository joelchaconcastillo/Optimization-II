...
def  PrimalDual(A, b, c, ite):
   ...
   for i in range(0,ite):
     alpha = np.random.uniform(0.01, 0.5, 1)
     alpha = 2
     S = diags(s)
     X = diags(x) 
...
def PrimalDual2(A, b, c, ite, alphamean, sigma):
   (m, n) = np.shape(A)
   (x, llambda, s) = InitialPoints(A,b,c) 
...
def LongStepPath(A, b, c, ite, errorgrad, errorfit, sigmamin, sigmamax, gamma):
   ...
   (x, llambda, s) = PrimalDual2(A, b, c, ite, 0.001, 0.1)
   I = identity(n, dtype=float)
   fitness = 1e100
   antnorm = 1e100

   for i in range(0,ite):
     sigma = 0.01#np.random.uniform(sigmamin, sigmamax, 1)
     S = diags(s)
     X = diags(x) 
     rs = csc_matrix.transpose(A).dot(llambda) + s - c
     rb = A.dot(x) - b
     ...
     xx = spsolve(K, -bb)
     alpha = 1e-200
     while (( x + alpha*xx[0:n] >= 0).all() and ( s + alpha*xx[(n+m): 2*n+m] >= 0).all() and np.linalg.norm(A.dot( x + alpha*xx[0:n] ) - b) < 1e-4) and np.linalg.norm( csc_matrix.transpose(A).dot(llambda + alpha*xx[n:(n+m)] )+ s + alpha*xx[(n+m): 2*n+m] -c) < 1e-4 :#and  np.multiply(x + alpha*xx[0:n],s + alpha*xx[(n+m): 2*n+m]    >=  Mu*gamma).any() : 
